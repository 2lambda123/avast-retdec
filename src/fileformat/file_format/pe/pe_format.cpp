...
const std::vector<std::string> stubDatabase = {
    ...
    "This program requires Microsoft Windows", "Win32 only"};

const std::map<std::string, std::size_t> visualBasicLibrariesMap = {
    {"msvbvm10.dll", 1}, {"msvbvm20.dll", 2}, {"msvbvm30.dll", 3},
    {"msvbvm40.dll", 4}, {"msvbvm50.dll", 5}, {"msvbvm60.dll", 6},
    {"vb40032.dll", 4}};
...
const std::unordered_set<std::string> usualSectionNames{
    ".00cfg",     ".BSS",       ".CLR_UEF",   ".CRT",
    ".DATA",      ".apiset",    ".arch",      ".autoload_text",
    ".bindat",    ".bootdat",   ".bss",       ".buildid",
    ".code",      ".complua",   ".cormeta",   ".cygwin_dll_common",
    ".data",      ".data1",     ".data2",     ".data3",
    ".debug  $F", ".debug  $P", ".debug  $S", ".debug  $T",
    ".debug",     ".didat",     ".didata",    ".drectve ",
    ".edata",     ".eh_fram",   ".export",    ".fasm",
    ".flat",      ".gfids",     ".giats",     ".gljmp",
    ".glue_7",    ".glue_7t",   ".idata",     ".idlsym",
    ".impdata",   ".import",    ".itext",     ".ndata",
    ".orpc",      ".pdata",     ".rdata",     ".reloc",
    ".rodata",    ".rsrc",      ".sbss",      ".script",
    ".sdata",     ".shared",    ".srdata",    ".stab",
    ".stabstr",   ".sxdata",    ".text",      ".text0",
    ".text1",     ".text2",     ".text3",     ".textbss",
    ".tls",       ".tls$",      ".udata",     ".vsdata",
    ".wixburn",   ".wpp_sf",    ".xdata",     "BSS",
    "CODE",       "DATA",       "DGROUP",     "INIT",
    "PAGE",       "Shared",     "edata",      "idata",
    "minATL",     "rdata",      "sdata",      "shared",
    "testdata",   "text"};

const std::unordered_set<std::string> usualPackerSections{
    "!EPack",   ".ASPack",  ".ByDwing", ".MPRESS1",   ".MPRESS2", ".MaskPE",
    ".RLPack",  ".RPCrypt", ".Themida", ".UPX0",      ".UPX1",    ".UPX2",
    ".Upack",   ".WWP32",   ".WWPACK",  ".adata",     ".aspack",  ".boom",
    ".ccg",     ".charmve", ".ecode",   ".edata",     ".enigma1", ".enigma2",
    ".gentee",  ".mackt",   ".mnbvcx1", ".mnbvcx2",   ".neolit",  ".neolite",
    ".nsp0",    ".nsp1",    ".nsp2",    ".packed",    ".perplex", ".petite",
    ".pinclie", ".rmnet",   ".seau",    ".sforce3",   ".shrink1", ".shrink2",
    ".shrink3", ".spack",   ".svkp",    ".taz",       ".tsuarch", ".tsustub",
    ".vmp0",    ".vmp1",    ".vmp2",    ".winapi",    ".y0da",    ".yP",
    "ASPack",   "BitArts",  "DAStub",   "FSG!",       "MEW",      "PEBundle",
    "PEC2",     "PEC2MO",   "PEC2TO",   "PECompact2", "PELOCKnt", "PEPACK!!",
    "PESHiELD", "ProCrypt", "RCryptor", "Themida",    "UPX!",     "UPX0",
    "UPX1",     "UPX2",     "UPX3",     "VProtect",   "WinLicen", "_winzip_",
    "kkrunchy", "lz32.dll", "nsp0",     "nsp1",       "nsp2",     "pebundle",
    "pec",      "pec1",     "pec2",     "pec3",       "pec4",     "pec5",
    "pec6",
...
const std::map<std::string, std::size_t> usualSectionCharacteristics{
    {".bss", (PELIB_IMAGE_SCN_CNT_UNINITIALIZED_DATA |
              PELIB_IMAGE_SCN_MEM_READ | PELIB_IMAGE_SCN_MEM_WRITE)},
    {".cormeta", PELIB_IMAGE_SCN_LNK_INFO},
    {".data", (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_MEM_READ |
               PELIB_IMAGE_SCN_MEM_WRITE)},
    {".debug", (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA |
                PELIB_IMAGE_SCN_MEM_READ | PELIB_IMAGE_SCN_MEM_DISCARDABLE)},
    {".drective", PELIB_IMAGE_SCN_LNK_INFO},
    {".edata",
     (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_MEM_READ)},
    {".idata", (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA |
                PELIB_IMAGE_SCN_MEM_READ | PELIB_IMAGE_SCN_MEM_WRITE)},
    ...
    {".rdata",
     (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_MEM_READ)},
    {".reloc", (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA |
                PELIB_IMAGE_SCN_MEM_READ | PELIB_IMAGE_SCN_MEM_DISCARDABLE)},
    {".rsrc",
     (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_MEM_READ)},
    {".sbss", (PELIB_IMAGE_SCN_CNT_UNINITIALIZED_DATA |
               PELIB_IMAGE_SCN_MEM_READ | PELIB_IMAGE_SCN_MEM_WRITE)},
    {".sdata", (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA |
                PELIB_IMAGE_SCN_MEM_READ | PELIB_IMAGE_SCN_MEM_WRITE)},
    {".srdata",
     (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_MEM_READ)},
    {".sxdata", PELIB_IMAGE_SCN_LNK_INFO},
    {".text", (PELIB_IMAGE_SCN_CNT_CODE | PELIB_IMAGE_SCN_MEM_EXECUTE |
               PELIB_IMAGE_SCN_MEM_READ)},
    {".tls", (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_MEM_READ |
              PELIB_IMAGE_SCN_MEM_WRITE)},
    {".tls$", (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_MEM_READ |
               PELIB_IMAGE_SCN_MEM_WRITE)},
    {".vsdata", (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA |
                 PELIB_IMAGE_SCN_MEM_READ | PELIB_IMAGE_SCN_MEM_WRITE)},
    {".xdata",
     (PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_MEM_READ)}};
 ...
 */
Symbol::Type getSymbolType(std::uint16_t link, std::uint16_t value,
 ...
 */
Symbol::UsageType getSymbolUsageType(std::uint8_t storageClass,
                                     std::uint8_t complexType) {
 ...
 */
PeFormat::PeFormat(const std::string &pathToFile,
                   const std::string &dllListFile, LoadFlags loadFlags)
 ...
 * @param loadFlags Load flags
 */
PeFormat::PeFormat(const std::uint8_t *data, std::size_t size,
                   LoadFlags loadFlags)
...
std::size_t PeFormat::getRichHeaderOffset(const std::string &plainFile) {
  ...
  if (prev != std::string::npos) {
    for (std::size_t i = 0, next = 0;
         (next = plainFile.find('\0', prev)) != std::string::npos; ++i) {
      if (i) {
        if (next != prev) {
          break;
        }
  ...
  return richOffset ? richOffset + getMzHeaderSize()
                    : STANDARD_RICH_HEADER_OFFSET;
}
 ...
 */
bool PeFormat::getResourceNodes(
...
void PeFormat::loadRichHeader() {
  ...
  bytesToString(bytes, plainText, getMzHeaderSize(),
                getPeHeaderOffset() - getMzHeaderSize());
  ...
  if (!header.isHeaderValid() && offset > getMzHeaderSize() &&
      getPeHeaderOffset() > getMzHeaderSize()) {
  ...
  if (!header.isHeaderValid() && !standardOffset &&
      STANDARD_RICH_HEADER_OFFSET < getPeHeaderOffset()) {
  ...
  if (!header.isHeaderValid()) {
    const auto nonStandardOffset =
        std::any_of(validStructOffsets.begin(), validStructOffsets.end(),
                    [&](const auto &off) {
                      return off != STANDARD_RICH_HEADER_OFFSET &&
                             off != this->getMzHeaderSize();
                    });
    ...
    for (const auto off : validStructOffsets) {
      if (off > maxOffset &&
          (!nonStandardOffset ||
           (off != STANDARD_RICH_HEADER_OFFSET && off != getMzHeaderSize()))) {
        maxOffset = off;
      }
    }
  ...
  auto crc32 = retdec::fileformat::getCrc32(decrypted_bytes.data(),
                                            decrypted_bytes.size());
  auto md5 = retdec::fileformat::getMd5(decrypted_bytes.data(),
                                        decrypted_bytes.size());
  auto sha256 = retdec::fileformat::getSha256(decrypted_bytes.data(),
                                              decrypted_bytes.size());
...
void PeFormat::loadVisualBasicHeader() {
  ...
  // first instruction is expected to be PUSH <vbHeaderAddress> (0x68 <b0> <b1>
  // <b2> <b3>)
  ...
  if (!getBytes(bytes, vbHeaderOffset, vbh.structureSize()) ||
      bytes.size() != vbh.structureSize()) {
    return;
  }
  ...
  std::memcpy(&vbh.languageDLL, static_cast<void *>(&bytes.data()[offset]),
              sizeof(vbh.languageDLL));
  offset += sizeof(vbh.languageDLL);
  std::memcpy(&vbh.backupLanguageDLL,
              static_cast<void *>(&bytes.data()[offset]),
              sizeof(vbh.backupLanguageDLL));
  ...
  if (vbh.projDescOffset != 0) {
    projDesc = retdec::utils::readNullTerminatedAscii(
        allBytes.data(), allBytes.size(), vbHeaderOffset + vbh.projDescOffset,
  ...
  if (vbh.helpFileOffset != 0) {
    helpFile = retdec::utils::readNullTerminatedAscii(
        allBytes.data(), allBytes.size(), vbHeaderOffset + vbh.helpFileOffset,
        VB_MAX_STRING_LEN, true);
    visualBasicInfo.setProjectHelpFile(helpFile);
  }
  if (vbh.projNameOffset != 0) {
    projName = retdec::utils::readNullTerminatedAscii(
        allBytes.data(), allBytes.size(), vbHeaderOffset + vbh.projNameOffset,
        VB_MAX_STRING_LEN, true);
  ...
  for (size_t i = 0;
       i < sizeof(vbh.backupLanguageDLL) && vbh.backupLanguageDLL[i]; i++) {
  ...
  if (getOffsetFromAddress(vbComDataRegistrationOffset,
                           vbh.COMRegisterDataAddr)) {
 ...
 * @return @c true if COM registration data was successfuly parsed, @c false
 * otherwise
 */
bool PeFormat::parseVisualBasicComRegistrationData(
  ...
  if (!getBytes(bytes, structureOffset, vbcrd.structureSize()) ||
      bytes.size() != vbcrd.structureSize()) {
    return false;
  }
  ...
  std::memcpy(&vbcrd.projCLSID, static_cast<void *>(&bytes.data()[offset]),
              sizeof(vbcrd.projCLSID));
  ...
  if (vbcrd.regInfoOffset != 0) {
    parseVisualBasicComRegistrationInfo(structureOffset + vbcrd.regInfoOffset,
                                        structureOffset);
  }
 ...
 * @param structureOffset Offset in file where the structure starts
 * @param comRegDataOffset Offset in file where the com registration data
 * structure starts
 * @return @c true if COM registration info was successfuly parsed, @c false
 * otherwise
 */
bool PeFormat::parseVisualBasicComRegistrationInfo(
    std::size_t structureOffset, std::size_t comRegDataOffset) {
  const auto &allBytes = getBytes();
  std::vector<std::uint8_t> bytes;
  std::size_t offset = 0;
  ...
  if (!getBytes(bytes, structureOffset, vbcri.structureSize()) ||
      bytes.size() != vbcri.structureSize()) {
    return false;
  }
  ...
  std::memcpy(&vbcri.objCLSID, static_cast<void *>(&bytes.data()[offset]),
              sizeof(vbcri.objCLSID));
  ...
  if (vbcri.isInterfaceFlag != 0 && vbcri.ifCLSIDOffset != 0 &&
      getBytes(bytes, comRegDataOffset + vbcri.ifCLSIDOffset, 16) &&
      bytes.size() == 16) {
    visualBasicInfo.setCOMObjectInterfaceCLSID(bytes.data());
  }

  if (vbcri.hasEvents != 0 && vbcri.eventCLSIDOffset != 0 &&
      getBytes(bytes, comRegDataOffset + vbcri.eventCLSIDOffset, 16) &&
      bytes.size() == 16) {
...
bool PeFormat::parseVisualBasicProjectInfo(std::size_t structureOffset) {
  ...
  if (!getBytes(bytes, structureOffset, vbpi.structureSize()) ||
      bytes.size() != vbpi.structureSize()) {
    return false;
  }
  ...
  std::memcpy(&vbpi.pathInformation, static_cast<void *>(&bytes.data()[offset]),
              sizeof(vbpi.pathInformation));
  ...
  projPath = retdec::utils::unicodeToAscii(vbpi.pathInformation,
                                           sizeof(vbpi.pathInformation));
 ...
 */
bool PeFormat::parseVisualBasicExternTable(std::size_t structureOffset,
                                           std::size_t nEntries) {
  const auto &allBytes = getBytes();
  std::vector<std::uint8_t> bytes;
  ...
  for (std::size_t i = 0; i < nEntries; i++) {
    ...
    if (!getBytes(bytes, structureOffset + i * entry.structureSize(),
                  entry.structureSize()) ||
    ...
    if (entry.type !=
        static_cast<std::uint32_t>(VBExternTableEntryType::external)) {
      continue;
    }
    ...
    if (getOffsetFromAddress(moduleNameOffset, entryData.moduleNameAddr)) {
      moduleName = retdec::utils::readNullTerminatedAscii(
          allBytes.data(), allBytes.size(), moduleNameOffset, VB_MAX_STRING_LEN,
          true);
    }
    ...
    if (getOffsetFromAddress(apiNameOffset, entryData.apiNameAddr)) {
      apiName = retdec::utils::readNullTerminatedAscii(
          allBytes.data(), allBytes.size(), apiNameOffset, VB_MAX_STRING_LEN,
          true);
    }
...
bool PeFormat::parseVisualBasicObjectTable(std::size_t structureOffset) {
  ...
  if (!getBytes(bytes, structureOffset, vbot.structureSize()) ||
      bytes.size() != vbot.structureSize()) {
    return false;
  }
  ...
  std::memcpy(&vbot.objectGUID, static_cast<void *>(&bytes.data()[offset]),
              sizeof(vbot.objectGUID));
  ...
  if (!visualBasicInfo.hasProjectName() &&
      getOffsetFromAddress(projectNameOffset, vbot.projectNameAddr)) {
    projName = retdec::utils::readNullTerminatedAscii(
        allBytes.data(), allBytes.size(), projectNameOffset, VB_MAX_STRING_LEN,
        true);
    visualBasicInfo.setProjectName(projName);
  }

  if (getOffsetFromAddress(objectDescriptorsOffset,
                           vbot.objectDescriptorsAddr)) {
 ...
 * @param structureOffset Offset in file where the public object descriptors
 * array starts
 * @param nObjects Number of objects in array
 * @return @c true if objects were successfuly parsed, @c false otherwise
 */
bool PeFormat::parseVisualBasicObjects(std::size_t structureOffset,
                                       std::size_t nObjects) {
  const auto &allBytes = getBytes();
  std::vector<std::uint8_t> bytes;
  ...
  for (std::size_t i = 0; i < nObjects; i++) {
    ...
    std::string objectName =
        readNullTerminatedAscii(allBytes.data(), allBytes.size(),
                                objectNameOffset, VB_MAX_STRING_LEN, true);
    ...
    if (getOffsetFromAddress(methodAddrOffset, vbpod.methodNamesAddr)) {
      for (std::size_t mIdx = 0; mIdx < vbpod.nMethods; mIdx++) {
        if (!getBytes(bytes, methodAddrOffset + mIdx * sizeof(std::uint32_t),
                      sizeof(std::uint32_t)) ||
...
void PeFormat::loadSections() {
  for (std::size_t i = 0, e = formatParser->getStoredNumberOfSections(); i < e;
       ++i) {
 ...
 *
 * Instance method @a loadSections() must be invoked before invocation of this
 * method
 */
  ...
  for (std::size_t i = 0, e = symTab.getNumberOfStoredSymbols(); i < e; ++i) {
    ...
    if (!link || link == std::numeric_limits<std::uint16_t>::max() ||
      ...
      if (link <= getNumberOfSections() && sections[link - 1]) {
        const auto a =
            sections[link - 1]->getAddress() + symTab.getSymbolValue(i);
        symbol->setAddress(a);
    ...
    symbol->setType(getSymbolType(link, symTab.getSymbolValue(i),
                                  symTab.getSymbolStorageClass(i)));
    symbol->setUsageType(getSymbolUsageType(symTab.getSymbolStorageClass(i),
                                            symTab.getSymbolTypeComplex(i)));
...
void PeFormat::loadImports() {
  ...
  for (std::size_t i = 0; formatParser->getImportedLibraryFileName(i, libname);
       ++i) {
  ...
  for (std::size_t i = 0;
       formatParser->getDelayImportedLibraryFileName(i, libname); ++i) {
  ...
  for (auto &&addressRange :
       formatParser->getImportDirectoryOccupiedAddresses()) {
...
void PeFormat::loadExports() {
  ...
  for (std::size_t i = 0, e = formatParser->getNumberOfExportedFunctions();
       i < e; ++i) {
    if (!formatParser->getExportedFunction(i, newExport))
      break;

    if (hasNonprintableChars(newExport.getName())) {
      newExport.setName("exported_function_" +
                        intToHexString(newExport.getAddress()));
    }
  ...
  for (auto &&addressRange :
       formatParser->getExportDirectoryOccupiedAddresses()) {
    nonDecodableRanges.insert(std::move(addressRange));
  }
}
...
void PeFormat::loadPdbInfo() {
  for (std::size_t i = 0, e = formatParser->getNumberOfDebugEntries(); i < e;
       ++i) {
    ...
    if ((prefix != "RSDS" && prefix != "NB10") ||
        (prefix == "RSDS" && size < MINIMAL_PDB_RSDS_INFO_LENGTH) ||
    ...
    const auto guidOffset =
        pointerToRawData + prefix.length() + (isRsds ? 0 : 4);
    std::uint64_t res1;
    if (isRsds) {
      std::uint64_t res2, res3, res4, res5;
      if (get4ByteOffset(guidOffset, res1) &&
          get2ByteOffset(guidOffset + 4, res2) &&
          get2ByteOffset(guidOffset + 6, res3) &&
          get2ByteOffset(guidOffset + 8, res4, getInverseEndianness()) &&
          getXByteOffset(guidOffset + 10, 6, res5, getInverseEndianness())) {
        pdbInfo->setGuid(
            toUpper(intToHexString(res1) + "-" + intToHexString(res2) + "-" +
                    intToHexString(res3) + "-" + intToHexString(res4) + "-" +
                    intToHexString(res5)));
      }
  ...
  for (auto &&addressRange :
       formatParser->getDebugDirectoryOccupiedAddresses()) {
    nonDecodableRanges.insert(std::move(addressRange));
  }
}
...
void PeFormat::loadResourceNodes(
  ...
  if (levels.empty() || !getDataDirectoryRelative(
                            PELIB_IMAGE_DIRECTORY_ENTRY_RESOURCE, rva, size)) {
    return;
  }
  ...
  for (std::size_t i = 0, e = resourceTree->getNumberOfLeafs(); i < e; ++i) {
    ...
    resource->setOffset(
        getImageLoader().getValidOffsetFromRva(leaf->getOffsetToData()));
...
void PeFormat::loadResources() {
  ...
  if (!getDataDirectoryRelative(PELIB_IMAGE_DIRECTORY_ENTRY_RESOURCE, rva,
                                size)) {
    return;
  }
  ...
  for (std::size_t i = 0, e = levels[0], nSft = 0, lSft = 0; i < e; ++i) {
    ...
    for (std::size_t j = 0, f = typeChild->getNumberOfChildren(); j < f; ++j) {
      ...
      for (std::size_t k = 0, g = nameChild->getNumberOfChildren(); k < g;
           ++k) {
        ...
        if (type == "Icon") {
          ...
          resourceTable->addResourceIcon(
              static_cast<ResourceIcon *>(resource.get()));
          ...
          resourceTable->addResourceIconGroup(
              static_cast<ResourceIconGroup *>(resource.get()));
          iconGroupIDcounter++;
        ...
        std::uint64_t dataOffset =
            getImageLoader().getValidOffsetFromRva(lanLeaf->getOffsetToData());
        ...
        if (resource->hasEmptyLanguage()) {
          ...
          resource->setLanguage(
              mapGetValueOrDefault(resourceLanguageMap, lId, ""));
        }
        resource->load(this);
        resourceTable->addResource(std::move(resource));
      }
  ...
  for (auto &&addressRange :
       formatParser->getResourceDirectoryOccupiedAddresses()) {
    nonDecodableRanges.insert(std::move(addressRange));
  }
}
...
static Certificate::Attributes getX509Attributes(Attributes attrs) {
  ...
  result.country =
      attrs.country.data
          ? std::string(reinterpret_cast<char *>(attrs.country.data),
                        attrs.country.len)
          : "";
  result.organization =
      attrs.organization.data
          ? std::string(reinterpret_cast<char *>(attrs.organization.data),
                        attrs.organization.len)
          : "";
  result.organizationalUnit =
      attrs.organizationalUnit.data
          ? std::string(reinterpret_cast<char *>(attrs.organizationalUnit.data),
                        attrs.organizationalUnit.len)
          : "";
  result.nameQualifier =
      attrs.nameQualifier.data
          ? std::string(reinterpret_cast<char *>(attrs.nameQualifier.data),
                        attrs.nameQualifier.len)
          : "";
  result.state = attrs.state.data
                     ? std::string(reinterpret_cast<char *>(attrs.state.data),
                                   attrs.state.len)
                     : "";
  result.commonName =
      attrs.commonName.data
          ? std::string(reinterpret_cast<char *>(attrs.commonName.data),
                        attrs.commonName.len)
          : "";
  result.serialNumber =
      attrs.serialNumber.data
          ? std::string(reinterpret_cast<char *>(attrs.serialNumber.data),
                        attrs.serialNumber.len)
          : "";
  result.locality =
      attrs.locality.data
          ? std::string(reinterpret_cast<char *>(attrs.locality.data),
                        attrs.locality.len)
          : "";
  result.title = attrs.title.data
                     ? std::string(reinterpret_cast<char *>(attrs.title.data),
                                   attrs.title.len)
                     : "";
  result.surname =
      attrs.surname.data
          ? std::string(reinterpret_cast<char *>(attrs.surname.data),
                        attrs.surname.len)
          : "";
  result.givenName =
      attrs.givenName.data
          ? std::string(reinterpret_cast<char *>(attrs.givenName.data),
                        attrs.givenName.len)
          : "";
  result.initials =
      attrs.initials.data
          ? std::string(reinterpret_cast<char *>(attrs.initials.data),
                        attrs.initials.len)
          : "";
  result.pseudonym =
      attrs.pseudonym.data
          ? std::string(reinterpret_cast<char *>(attrs.pseudonym.data),
                        attrs.pseudonym.len)
          : "";
  result.generationQualifier =
      attrs.generationQualifier.data
          ? std::string(
                reinterpret_cast<char *>(attrs.generationQualifier.data),
                attrs.generationQualifier.len)
          : "";
  result.emailAddress =
      attrs.emailAddress.data
          ? std::string(reinterpret_cast<char *>(attrs.emailAddress.data),
                        attrs.emailAddress.len)
          : "";
...
static std::vector<Certificate> getCertificates(CertificateArray *arr) {
  ...
  for (std::size_t i = 0; i < arr->count; ++i) {
    ...
    new_cert.validSince =
        cert->not_before ? time_to_string(cert->not_before) : "";
    new_cert.validUntil =
        cert->not_after ? time_to_string(cert->not_after) : "";
    ...
    if (cert->sha256.data)
      bytesToHexString(cert->sha256.data, cert->sha256.len,
                       new_cert.sha256Digest);
...
static void writeSignerInfo(::Signer *signer, DigitalSignature &signature) {
  ...
  signature.signer.digestAlgorithm =
      signer->digest_alg ? signer->digest_alg : "";
  if (signer->digest.data)
    bytesToHexString(signer->digest.data, signer->digest.len,
...
static Signer getCountersigner(Countersignature *counter) {
  ...
  countersigner.digestAlgorithm =
      counter->digest_alg ? counter->digest_alg : "";
  countersigner.signingTime =
      counter->sign_time ? time_to_string(counter->sign_time) : "";
  if (counter->digest.data)
    bytesToHexString(counter->digest.data, counter->digest.len,
                     countersigner.digest);

  // If there is any verification error, export it as a proper message
  if (counter->verify_flags != COUNTERSIGNATURE_VFY_VALID)
    countersigner.warnings.emplace_back(
        countersigFlagToString(counter->verify_flags));

  return countersigner;
}

static std::vector<DigitalSignature>
authenticodeToSignatures(AuthenticodeArray *arr, const PeFormat *file) {
  if (!arr || !file)
    return {};
  ...
  for (std::size_t i = 0; i < arr->count; ++i) {
    ...
    if (auth->digest.data)
      bytesToHexString(auth->digest.data, auth->digest.len,
                       signature.signedDigest);
    if (auth->file_digest.data)
      bytesToHexString(auth->file_digest.data, auth->file_digest.len,
                       signature.fileDigest);
    ...
    if (auth->verify_flags != AUTHENTICODE_VFY_VALID) {
      signature.warnings.emplace_back(
          authenticodeFlagToString(auth->verify_flags));
    }
...
void PeFormat::loadCertificates() {
  ...
  AuthenticodeArray *auth =
      parse_authenticode(this->getBytesData(), this->getFileLength());
...
void PeFormat::loadTlsInformation() {
  std::uint64_t rva = 0, size = 0;
  if (!getDataDirectoryRelative(PELIB_IMAGE_DIRECTORY_ENTRY_TLS, rva, size) ||
      size == 0) {
    return;
  }
...
void PeFormat::loadDotnetHeaders() {
  ...
  // If our file contains CLR header, then use it. Note that .NET framework
  // doesn't verify the OPTIONAL_HEADER::NumberOfRvaAndSizes, so we must do it
  // the same way.
  std::uint64_t comHeaderAddress, comHeaderSize;
  if (getComDirectoryRelative(comHeaderAddress, comHeaderSize) &&
      comHeaderSize) {
    clrHeader = formatParser->getClrHeader();
    metadataHeaderAddress = formatParser->getImageBaseAddress() +
                            clrHeader->getMetadataDirectoryAddress();
  } else {
    return;
  }

  // If not, then try to guess whether the file could possibly be .NET file
  // based on imports and try to search for metadata header LZ: Don't. This will
  // lead to the situation when totally unrelated .NET metadata will be read
  // from the binary, for example from a binary embedded in resources or in
  // overlay. Sample:
  ...
  */
  ...
  // This explicit initialization needs to be here, because clang 4.0 has bug in
  // optimizer and it causes problem in valgrind.
  std::uint64_t signature = 0;
  if (!get4Byte(metadataHeaderAddress, signature) ||
      signature != MetadataHeaderSignature) {
    return;
  }
  ...
  metadataHeader->setAddress(metadataHeaderAddress -
                             formatParser->getImageBaseAddress());
...
PeTimestamps PeFormat::getTimestamps() const {
  ...
  // 5. then following PointerToRawData we can find another TimeDateStamp in
  // Pdb 2.0 structure
  ...
  for (int i = 0; i < debugDir.calcNumberOfEntries(); ++i) {
    ...
    if (type == PELIB_IMAGE_DEBUG_INFO_CODEVIEW) {
      ...
      const std::uint8_t *signature =
          reinterpret_cast<const std::uint8_t *>("NB10");
  ...
  if (root) {
    ...
    for (std::size_t i = 0, e = root->getNumberOfChildren(); i < e; ++i) {
      ...
      const ResourceNode *directory =
          dynamic_cast<const ResourceNode *>(child->getNode());
      if (!directory)
        continue;
      ...
      for (int i = 0; i < directory->getNumberOfChildren(); i++) {
        ...
        const ResourceNode *second_directory =
            dynamic_cast<const ResourceNode *>(second_child->getNode());
        if (!second_directory)
          continue;

        timestamps.resourceTime.push_back(
            (second_directory->getTimeDateStamp()));
      }
    }
  }
 ...
 */
std::vector<std::tuple<const std::uint8_t *, std::size_t>>
PeFormat::getDigestRanges() const {
  ...
  std::size_t secDirFileOffset =
      formatParser->getSecurityDirFileOffset(); // offset of security directory
                                                // record in PE header
  std::size_t secDirOffset = formatParser->getSecurityDirRva();
  std::size_t secDirSize = formatParser->getSecurityDirSize();

  // To prevent crashes on unordinary binaries, we need to sort these offsets
  // (together with sizes, but they are unimportant for sorting) Usually,
  // checksum is first, then security directory header and then security
  // directory There are a few binaries where this order is not followed
  ...
  for (auto &offsetSize : offsets) {
    // If the length of the range is bigger than the amount of data we have
    // available, then sanitize the length
    ...
    result.emplace_back(bytes.data() + lastOffset,
                        offsetSize.first - lastOffset);
 ...
 */
void PeFormat::parseMetadataStream(std::uint64_t baseAddress,
  ...
  if (!get1Byte(address + 4, majorVersion) ||
      ...
      !get8Byte(address + 8, valid) || !get8Byte(address + 16, sorted)) {
    return;
  }
  ...
  // 'heapOffsetSizes' define whether we should use std::uint16_t or
  // dstd::uint16_t for indexes into different streams
  ...
  // Tables that are present in the stream are marked with bit 1 in 'valid'
  // attribute At first we need to create all tables with their sizes because we
  // don't know how many of them is there, so we don't know where to start to
  // parse table content.
  ...
  for (std::size_t i = 0; i < 64; ++i) {
    if ((valid >> i) & 1) {
      ...
      // If the size of the metadata table would be larger than there are data
      // available, we just end. This must be corrupted.
      if (tableSize > getLoadedFileLength()) {
        return;
      }

      metadataStream->addMetadataTable(static_cast<MetadataTableType>(i),
                                       tableSize);
      currentAddress += 4;
    }
  }
  // ExtraData flags means there is extra 4 bytes at the end Rows array that
  // contaisn the rows sizes I don't see anything about in at ECMA-335, but I
  // can see in real samples and in IlSpy source that understands it and
  // correctly decompiles, sample:
  ...
  for (std::size_t i = 0; i < 64; ++i) {
    auto table =
        metadataStream->getMetadataTable(static_cast<MetadataTableType>(i));
    if (table == nullptr) {
      continue;
    }
 ...
 * @param size Size of stream.
 */
void PeFormat::parseBlobStream(std::uint64_t baseAddress, std::uint64_t offset,
                               std::uint64_t size) {
  std::vector<std::uint8_t> data;
  auto address = baseAddress + offset;
 ...
 * @param size Size of stream.
 */
void PeFormat::parseGuidStream(std::uint64_t baseAddress, std::uint64_t offset,
                               std::uint64_t size) {
 ...
 * @param size Size of stream.
 */
void PeFormat::parseStringStream(std::uint64_t baseAddress,
                                 std::uint64_t offset, std::uint64_t size) {
 ...
 * @param size Size of stream.
 */
void PeFormat::parseUserStringStream(std::uint64_t /* baseAddress*/,
                                     std::uint64_t offset, std::uint64_t size) {
 ...
 */
template <typename T>
void PeFormat::parseMetadataTable(BaseMetadataTable *table,
...
void PeFormat::detectModuleVersionId() {
  ...
  if (!metadataStream || !guidStream ||
      !metadataStream->hasTable(MetadataTableType::Module)) {
    return;
  }

  auto moduleTable = static_cast<const MetadataTable<DotnetModule> *>(
      metadataStream->getMetadataTable(MetadataTableType::Module));
...
void PeFormat::detectTypeLibId() {
  ...
  auto typeRefTable = static_cast<const MetadataTable<TypeRef> *>(
      metadataStream->getMetadataTable(MetadataTableType::TypeRef));
  auto memberRefTable = static_cast<const MetadataTable<MemberRef> *>(
      metadataStream->getMetadataTable(MetadataTableType::MemberRef));
  auto customAttributeTable =
      static_cast<const MetadataTable<CustomAttribute> *>(
  ...
  if (!typeRefTable || !memberRefTable || !customAttributeTable ||
      !assemblyRefTable) {
    return;
  }

  // First find the index of GuidAttribute type reference in TypeRef table,
  // which has ResolutionScope as mscorlib
  ...
  for (std::size_t i = 1; i <= typeRefTable->getNumberOfRows(); ++i) {
    ...
    auto assemblyRef =
        assemblyRefTable->getRow(typeRefRow->resolutionScope.getIndex());
    if (!assemblyRef) {
      continue;
    }

    std::string assemblyName;
    if (!stringStream->getString(assemblyRef->name.getIndex(), assemblyName) ||
        assemblyName != "mscorlib") {
      continue;
    }

    std::string typeName;
    if (stringStream->getString(typeRefRow->typeName.getIndex(), typeName) &&
        typeName == "GuidAttribute") {
      guidTypeRef = i;
      break;
    }
  }
  ...
  for (std::size_t i = 1; i <= customAttributeTable->getNumberOfRows(); ++i) {
    ...
    if (customAttributeRow->parent.getTable(parent_table) &&
        parent_table != MetadataTableType::Assembly) {
      continue;
    }
    ...
    if (customAttributeRow->type.getIndex() == guidMemberRef) {
      ...
      auto typeLibData =
          blobStream->getElement(customAttributeRow->value.getIndex());
      if (typeLibData.size() < 3) {
        continue;
      }

      // Custom attributes contain one std::uint16_t 0x0001 at the beginning so
      // we skip it, followed by length of the string, which is GUID we are
      // looking for
      auto length = typeLibData[2];
      typeLibId = retdec::utils::toLower(std::string(
          reinterpret_cast<const char *>(typeLibData.data() + 3), length));
...
/**
 * Detects and reconstructs .NET types such as classes, methods, fields,
 * properties etc.
 */
void PeFormat::detectDotnetTypes() {
  DotnetTypeReconstructor reconstructor(metadataStream.get(),
                                        stringStream.get(), blobStream.get());
...
/**
 * Detects possible metadata header structure. It first searches for metadata
 * header signature `0x424A5342`. If it finds this signature, it then tries to
 * look further for possible stream names.
 *
 * @return Possible metadata header address, 0 if unsuccessful.
 */
std::uint64_t PeFormat::detectPossibleMetadataHeaderAddress() const {
  const auto possibleStreamNames = {"#~",    "#-",    "#Strings",
                                    "#Blob", "#GUID", "#US"};
  ...
  for (const auto *sec : sections) {
    ...
    for (std::uint64_t inSecAddress = address;
         !metadataHeaderFound && inSecAddress < sec->getEndAddress();
         ++inSecAddress) {
      ...
      if (signature == MetadataHeaderSignature) {
        ...
        auto firstStreamNameAddress =
            inSecAddress + 16 // skip metadata header fields
...
void PeFormat::computeTypeRefHashes() {
  ...
  auto moduleTable = static_cast<const MetadataTable<DotnetModule> *>(
      metadataStream->getMetadataTable(MetadataTableType::Module));
  auto moduleRefTable = static_cast<const MetadataTable<ModuleRef> *>(
      metadataStream->getMetadataTable(MetadataTableType::ModuleRef));
  auto assemblyRefTable = static_cast<const MetadataTable<AssemblyRef> *>(
      metadataStream->getMetadataTable(MetadataTableType::AssemblyRef));
  ...
  for (std::size_t i = 1; i <= typeRefTable->getNumberOfRows(); ++i) {
    ...
    if (stringStream->getString(typeRefRow->typeNamespace.getIndex(),
                                nameSpace) &&
        !nameSpace.empty()) {
      validNameSpace = true;
    }
    ...
    if (typeRefRow->resolutionScope.getTable(resolutionScopeType)) {
      ...
      case MetadataTableType::TypeRef: {
        auto typeRef =
            typeRefTable->getRow(typeRefRow->resolutionScope.getIndex());
        if (typeRef &&
            stringStream->getString(typeRef->typeName.getIndex(),
                                    referencedName) &&
      ...
      case MetadataTableType::Module: {
        if (moduleTable) {
          auto module =
              moduleTable->getRow(typeRefRow->resolutionScope.getIndex());
          if (module &&
              stringStream->getString(module->name.getIndex(),
                                      referencedName) &&
              !referencedName.empty()) {
            referencedName += "M";
            validReferencedName = true;
      ...
      case MetadataTableType::ModuleRef: {
        if (moduleRefTable) {
          auto moduleRef =
              moduleRefTable->getRow(typeRefRow->resolutionScope.getIndex());
          if (moduleRef &&
              stringStream->getString(moduleRef->name.getIndex(),
                                      referencedName) &&
              !referencedName.empty()) {
            referencedName += "MR";
            validReferencedName = true;
      ...
      case MetadataTableType::AssemblyRef: {
        if (assemblyRefTable) {
          auto assemblyRef =
              assemblyRefTable->getRow(typeRefRow->resolutionScope.getIndex());
          if (assemblyRef &&
              stringStream->getString(assemblyRef->name.getIndex(),
                                      referencedName) &&
              !referencedName.empty()) {
            referencedName += "AR";
            validReferencedName = true;
  ...
  typeRefHashCrc32 = retdec::fileformat::getCrc32(typeRefHashBytes.data(),
                                                  typeRefHashBytes.size());
  typeRefHashMd5 = retdec::fileformat::getMd5(typeRefHashBytes.data(),
                                              typeRefHashBytes.size());
  typeRefHashSha256 = retdec::fileformat::getSha256(typeRefHashBytes.data(),
                                                    typeRefHashBytes.size());
}
...
bool PeFormat::hasMixedEndianForDouble() const { return false; }

/**
 * Get declared length of file. This length may be shorter or longer than real
 * length of file.
...
bool PeFormat::areSectionsValid() const { return true; }

bool PeFormat::isObjectFile() const { return false; }

bool PeFormat::isDll() const { return formatParser->isDll(); }

bool PeFormat::isExecutable() const { return !isDll(); }
...
std::size_t PeFormat::getDeclaredNumberOfSegments() const { return 0; }

std::size_t PeFormat::getSectionTableOffset() const {
  std::size_t res = getPeHeaderOffset() + formatParser->getSizeOfPeSignature() +
                    PELIB_IMAGE_FILE_HEADER::size() + getOptionalHeaderSize();
...
std::size_t PeFormat::getSegmentTableOffset() const { return 0; }

std::size_t PeFormat::getSegmentTableEntrySize() const { return 0; }
 ...
 *
 * In some cases (e.g. FSG packer), offset of PE signature may be inside MZ
 * header and therefore this method may return lesser number that method @a
 * getMzHeaderSize().
 */
...
bool PeFormat::isMissingDependency(std::string dllName) const {
  ...
  // In Windows 7+, DLLs beginning with name "api-" or "ext-" are resolved by
  // API set; We consider them always existing
  if ((dllName.length() > 4) && (!strncmp(dllName.c_str(), "api-", 4) ||
                                 !strncmp(dllName.c_str(), "ext-", 4)))
    return false;
 ...
 */
bool PeFormat::dllListFailedToLoad() const { return errorLoadingDllList; }
...
bool PeFormat::initDllList(const std::string &dllListFile) {
  ...
  if (dllListFile.length()) {
    ...
    while (stream) {
      ...
      std::transform(oneLine.begin(), oneLine.end(), oneLine.begin(),
                     ::tolower);
...
bool PeFormat::isDotNet() const {
  ...
  std::uint32_t numberOfRvaAndSizes =
      getImageLoader().getOptionalHeader().NumberOfRvaAndSizes;
  ...
  } else if (!isDll()) { // If 32 bit check if first 2 bytes at entry point are
                         // 0xFF 0x25
 ...
 * @return @c true if input file contains packed CIL/.NET code, @c false
 * otherwise
 */
 ...
 * @param version Into this parameter is stored version of Visual Basic, or @c 0
 * if version was not detected
 * @return @c true if input file original language is Visual Basic, @c false
 * otherwise
 */
bool PeFormat::isVisualBasic(std::uint64_t &version) const {
  version = 0;
  return importTable &&
         std::any_of(visualBasicLibrariesMap.begin(),
                     visualBasicLibrariesMap.end(), [&](const auto &item) {
                       if (this->importTable
 ...
 * @return @c true if file is DLL and flags are successfully detected, @c false
 * otherwise
 */
 ...
 * @return @c true if number of blocks is successfully detected, @c false
 * otherwise
 *
...
bool PeFormat::getNumberOfBaseRelocationBlocks(std::uint64_t &relocs) const {
  ...
  if (!getDataDirectoryRelative(PELIB_IMAGE_DIRECTORY_ENTRY_BASERELOC, addr,
                                size) ||
      !addr) {
    return false;
  }
 ...
 * @return @c true if number of relocations is successfully detected, @c false
 * otherwise
 *
 * If function returns @c false, @a relocs is left unchanged
 */
 ...
 * @param relAddr Into this parameter is stored relative virtual address of
 * directory
 ...
 */
bool PeFormat::getDataDirectoryRelative(std::uint64_t index,
                                        std::uint64_t &relAddr,
 ...
 * Get data directory
 * @param index Index of selected directory
 * @param absAddr Into this parameter is stored absolute virtual address of
 * directory
 * @param size Into this parameter is stored size of directory
 * @return @c true if index of selected directory is valid, @c false otherwise
 *
 ...
 */
bool PeFormat::getDataDirectoryAbsolute(std::uint64_t index,
...
/**
 * Special for .NET data directory to correctly process data directory on 32-bit
 * binaries
 * @param relAddr Into this parameter is stored relative virtual address of
 * directory
 * @param size Into this parameter is stored size of directory
 ...
 * If method returns @c false, @a relAddr and @a size are left unchanged.
 */
bool PeFormat::getComDirectoryRelative(std::uint64_t &relAddr,
                                       std::uint64_t &size) const {
 ...
 *
 * If file has more sections with name equal to @a secName, then is returned
 * first such section.
 */
...
const CLRHeader *PeFormat::getCLRHeader() const { return clrHeader.get(); }
...
const BlobStream *PeFormat::getBlobStream() const { return blobStream.get(); }

const GuidStream *PeFormat::getGuidStream() const { return guidStream.get(); }
...
const std::string &PeFormat::getTypeLibId() const { return typeLibId; }

const std::vector<std::shared_ptr<DotnetClass>> &
PeFormat::getDefinedDotnetClasses() const {
  return definedClasses;
}

const std::vector<std::shared_ptr<DotnetClass>> &
PeFormat::getImportedDotnetClasses() const {
...
void PeFormat::scanForSectionAnomalies(unsigned anomaliesLimit) {
  ...
  if (getEpAddress(epAddr)) {
    auto *epSec =
        dynamic_cast<const PeCoffSection *>(getSectionFromAddress(epAddr));
    if (epSec) {
      // scan EP in last section
      const PeCoffSection *lastSec =
          (nSecs) ? getPeSection(nSecs - 1) : nullptr;
      if (epSec == lastSec) {
        anomalies.emplace_back("EpInLastSection",
                               "Entry point in the last section");
      }

      // scan EP in writable section
      if (epSec->getPeCoffFlags() & PELIB_IMAGE_SCN_MEM_WRITE) {
        anomalies.emplace_back("EpInWritableSection",
                               "Entry point in writable section");
      }
      // if we can't get valid offset then the EP is outside of the physical
      // file
      std::uint64_t epOffset = 0;
      if (!getEpOffset(epOffset)) {
        anomalies.emplace_back("EpInMemoryOnly",
                               "Entry point in memory-only part of a section");
      }
    } else {
      // scan EP outside mapped sections
      anomalies.emplace_back("EpOutsideSections",
  ...
  for (std::size_t i = 0; i < nSecs; i++) {
    ...
    const std::string msgName =
        (name.empty()) ? std::to_string(sec->getIndex()) : name;
    ...
    if (!name.empty()) {
      ...
      if (!duplName && secNames.find(name) != secNames.end()) {
        anomalies.emplace_back("DuplicitSectionNames",
                               "Multiple sections with name " +
                                   replaceNonprintableChars(name));
      ...
      if (usualPackerSections.find(name) != usualPackerSections.end()) {
        if (!duplName) {
          anomalies.emplace_back("PackerSectionName",
      ...
      // scan for unusual section names
        ...
        if (!duplName) {
          anomalies.emplace_back("UnusualSectionName",
      ...
      if (characIt != usualSectionCharacteristics.end() &&
          characIt->second != flags) {
        anomalies.emplace_back("UnusualSectionFlags",
                               "Section " + pname +
    ...
    if (sec->getSizeInFile() >= 100000000UL) {
      anomalies.emplace_back("LargeSection",
    ...
    if ((flags & PELIB_IMAGE_SCN_CNT_UNINITIALIZED_DATA) &&
      ...
      anomalies.emplace_back("UninitSectionHasData",
                             "Section " + pmsgName +
                                 " is marked uninitialized but contains data");
    }
    ...
    for (std::size_t j = i + 1; j < nSecs; j++) {
      ...
      if (sec->getSizeInFile() != 0) {
        ...
        if ((secStart <= cmpSecStart && cmpSecStart < secEnd) ||
          ...
          const std::string cmpMsgName =
              cmpName.empty() ? std::to_string(cmpSec->getIndex()) : cmpName;
          anomalies.emplace_back("OverlappingSections",
                                 "Sections " + pmsgName + " and " +
                                     replaceNonprintableChars(cmpMsgName) +
                                     " overlap");
        }
      }
    }
...
void PeFormat::scanForResourceAnomalies() {
  ...
  for (std::size_t i = 0; i < resourceTable->getNumberOfResources(); i++) {
    ...
    std::size_t nameId;
    std::string msgName =
        (res->getNameId(nameId)) ? std::to_string(nameId) : "<unknown>";

    // scan for resource size over 100MB
    if (res->getSizeInFile() >= 100000000UL) {
      anomalies.emplace_back("LargeResource",
                             "Resource " + replaceNonprintableChars(msgName) +
                                 " has size over 100MB");
    }

    // scan for resource stretched over multiple sections
    std::uint64_t resAddr;
    if (res->isValidOffset() &&
        getAddressFromOffset(resAddr, res->getOffset()) &&
        isObjectStretchedOverSections(resAddr, res->getSizeInFile())) {
      anomalies.emplace_back("StretchedResource",
                             "Resource " + replaceNonprintableChars(msgName) +
                                 " is stretched over multiple sections");
    }
  }
}
...
void PeFormat::scanForImportAnomalies() {
  ...
  for (const auto &impRange :
    ...
    if (getAddressFromOffset(impAddr, impRange.getStart()) &&
      ...
      anomalies.emplace_back("StretchedImportTable",
                             "Import " + replaceNonprintableChars(msgName) +
                                 " is stretched over multiple sections");
    }
  }
}
...
void PeFormat::scanForExportAnomalies() {
  ...
  for (const auto &expRange :
    ...
    if (getAddressFromOffset(expAddr, expRange.getStart()) &&
      ...
      anomalies.emplace_back("StretchedExportTable",
                             "Export " + replaceNonprintableChars(msgName) +
                                 " is stretched over multiple sections");
    }
  }
}
...
void PeFormat::scanForOptHeaderAnomalies() {
  ...
  if (!formatParser->isSizeOfHeaderMultipleOfFileAlignment()) {
    anomalies.emplace_back(
        "SizeOfHeaderNotAligned",
        "SizeOfHeader is not aligned to multiple of FileAlignment");
  }
}
