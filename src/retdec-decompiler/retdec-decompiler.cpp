...
void limitMaximalMemoryIfRequested(const retdec::config::Parameters &params) {
  if (params.isMaxMemoryLimitHalfRam()) {
    ...
    if (!ok) {
      throw std::runtime_error("failed to limit maximal memory to " +
                               std::to_string(lim));
    }
  }
}
...
/**
 * TODO: this function is exact copy of the function located in
 * retdec/retdec.cpp. The reason for this is that right now creation of correct
 * interface that would hold this function is much more time expensive than hard
 * copy.
 *
...
void setLogsFrom(const retdec::config::Parameters &params) {
  ...
  outLog.reset(logFile.empty() ? new Logger(std::cout, verbose)
...
int decompile(retdec::config::Config &config, ProgramOptions &po) {
  ...
  if (fat.isValid()) {
    ...
    if (config.architecture.isKnown()) {
      if (!fat.extractArchiveForFamily(config.architecture.getName(),
                                       extractedFile)) {
        std::stringstream ss;
  ...
  if (po.arIdx || !po.arName.empty()) {
    ...
    bool ok = true;
    std::string errMsg;
    retdec::ar_extractor::ArchiveWrapper arw(config.parameters.getInputFile(),
                                             ok, errMsg);
    ...
    if (ok && arw.isThinArchive()) {
      ...
      Log::error() << "Error: File is a thin archive and cannot be decompiled."
                   << std::endl;
      return EXIT_FAILURE;
    ...
    if (!ok &&
        retdec::ar_extractor::isArchive(config.parameters.getInputFile())) {
      Log::error() << "This file is an archive!" << std::endl;
      Log::error() << "Error: The input archive has invalid format."
                   << std::endl;
      return EXIT_FAILURE;
    }
  }
  ...
  char *uargv[4] = {unpackArgs[0].data(), unpackArgs[1].data(),
                    unpackArgs[2].data(), unpackArgs[3].data()};
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (config.parameters.isTimeout()) {
      std::packaged_task<int(retdec::config::Config &, ProgramOptions &)> task(
          decompile);
