...
getOutputStream(const std::string &outputFile) {
  // Open the file.
  std::error_code ec;
  auto out = std::make_unique<ToolOutputFile>(outputFile, ec, sys::fs::F_None);
  if (ec) {
...
void printErrorUnsupportedObject(const std::string &typeOfObjectsSingular,
  ...
  if (!supportedObjects.empty()) {
    Log::error() << Log::Error << "Invalid name of the "
                 << typeOfObjectsSingular
                 << " (supported names are: " << supportedObjects << ")."
                 << std::endl;
  } else {
    Log::error() << Log::Error << "There are no available "
                 << typeOfObjectsPlural
...
static RegisterPass<LlvmIr2Hll> X("retdec-llvmir2hll", "LLVM IR -> HLL",
 ...
 */
LlvmIr2Hll::LlvmIr2Hll(retdec::config::Config *c) : ModulePass(ID) {
  setConfig(c);
}

void LlvmIr2Hll::setConfig(retdec::config::Config *c) { globalConfig = c; }
...
bool LlvmIr2Hll::initialize(llvm::Module &m) {
  ...
  Log::phase("creating the used HLL writer [" + TargetHLL + "]", Log::SubPhase);

  // Output stream into which the generated code will be emitted.
  if (outStringStream) {
    hllWriter = llvmir2hll::HLLWriterFactory::getInstance()
                    .createObject<raw_string_ostream &>(
                        TargetHLL, *outStringStream,
                        globalConfig->parameters.getOutputFormat());
  } else {
    ...
    hllWriter = llvmir2hll::HLLWriterFactory::getInstance()
                    .createObject<raw_pwrite_stream &>(
                        TargetHLL, outFile->os(),
                        globalConfig->parameters.getOutputFormat());
  }

  if (!hllWriter) {
    printErrorUnsupportedObject<llvmir2hll::HLLWriterFactory>("target HLL",
                                                              "target HLLs");
    return false;
  }

  // Instantiate the requested alias analysis and make sure it exists.
  Log::phase("creating the used alias analysis [" + oAliasAnalysis + "]",
             Log::SubPhase);
  ...
  Log::phase("creating the used call info obtainer [" +
  ...
  // sure it exists.
  Log::phase("creating the used evaluator of arithmetical expressions [" +
  ...
  // exists.
  Log::phase("creating the used variable names generator [" + oVarNameGen + "]",
             Log::SubPhase);
  varNameGen = llvmir2hll::VarNameGenFactory::getInstance().createObject(
      oVarNameGen, VarNameGenPrefix);
  if (!varNameGen) {
...
void LlvmIr2Hll::createSemanticsFromParameter() {
  if (oSemantics.empty() || oSemantics == "-") {
    ...
    Log::phase("creating the used semantics [none]", Log::SubPhase);
    semantics = llvmir2hll::DefaultSemantics::create();
  } else {
    // Use the given semantics.
    Log::phase("creating the used semantics [" + oSemantics + "]",
               Log::SubPhase);
    semantics =
        llvmir2hll::CompoundSemanticsBuilder::build(split(oSemantics, ','));
  }
}
...
void LlvmIr2Hll::createSemanticsFromLLVMIR() {
  ...
  Log::phase("creating the used semantics [" + usedSemantics + "]",
             Log::SubPhase);
  semantics =
      llvmir2hll::CompoundSemanticsBuilder::build(split(usedSemantics, ','));
}
...
bool LlvmIr2Hll::loadConfig() {
  ...
  try {
    config =
        llvmir2hll::JSONConfig::fromString(globalConfig->generateJsonString());
    return true;
...
void LlvmIr2Hll::saveConfig() {
  if (globalConfig && !globalConfig->parameters.getOutputConfigFile().empty()) {
...
bool LlvmIr2Hll::convertLLVMIRToBIR() {
  ...
  std::string moduleName = ForcedModuleName.empty()
                               ...
                               : ForcedModuleName;
  resModule = llvm2BIRConverter->convert(llvmModule, moduleName, semantics,
                                         config, Debug);
 ...
 */
void LlvmIr2Hll::initAliasAnalysis() { aliasAnalysis->init(resModule); }
...
void LlvmIr2Hll::runOptimizations() {
  ShPtr<llvmir2hll::OptimizerManager> optManager(
      new llvmir2hll::OptimizerManager(
          ...
          hllWriter, llvmir2hll::ValueAnalysis::create(aliasAnalysis, true),
          cio, arithmExprEvaluator, Debug));
 ...
 *        variable renamer.
 */
void LlvmIr2Hll::renameVariables() { varRenamer->renameVars(resModule); }
...
void LlvmIr2Hll::findPatterns() {
  ...
  ShPtr<llvmir2hll::PatternFinderRunner> pfr(instantiatePatternFinderRunner());
...
void LlvmIr2Hll::emitCFGs() {
  ...
  if (!hasItem(availCFGWriters, oCFGWriter)) {
    printErrorUnsupportedObject<llvmir2hll::CFGWriterFactory>("CFG writer",
                                                              "CFG writers");
    return;
  }
  ...
  for (auto i = resModule->func_definition_begin(),
    ...
    std::string fileName(globalConfig->parameters.getOutputFile() + ".cfg." +
                         (*i)->getName() + "." + fileExt);
    std::ofstream out(fileName.c_str());
    if (!out) {
      Log::error() << Log::Error << "Cannot open " + fileName + " for writing."
                   << std::endl;
      return;
    }
    ...
    ShPtr<llvmir2hll::CFGWriter> writer(
        cfgwf.createObject<ShPtr<llvmir2hll::CFG>, std::ostream &>(
            oCFGWriter, cfgBuilder->getCFG(*i), out));
...
void LlvmIr2Hll::emitCG() {
  ...
  llvmir2hll::StringVector availCGWriters(inst.getRegisteredObjects());
  if (!hasItem(availCGWriters, std::string(oCGWriter))) {
    printErrorUnsupportedObject<llvmir2hll::CGWriterFactory>("CG writer",
                                                             "CG writers");
    return;
  }
  ...
  // Open the output file.
  std::string fileName(globalConfig->parameters.getOutputFile() + ".cg." +
                       fileExt);
  std::ofstream out(fileName.c_str());
  if (!out) {
    Log::error() << Log::Error << "Cannot open " + fileName + " for writing."
  ...
  ASSERT_MSG(writer, "instantiation of the requested CG writer `"
                         << oCGWriter << "` failed");
  writer->emitCG();
}
...
LlvmIr2Hll::parseListOfOpts(const std::string &opts) const {
...
LlvmIr2Hll::instantiatePatternFinders(
  ...
  for (const auto &pfId : pfsIds) {
    auto &inst = llvmir2hll::PatternFinderFactory::getInstance();
    ShPtr<llvmir2hll::PatternFinder> pf(inst.createObject(pfId, va, cio));
    if (!pf && Debug) {
      Log::error() << Log::Warning
                   << "the requested pattern finder '" + pfId +
                          "' does not exist"
                   << std::endl;
    } else {
      pfs.push_back(pf);
