...
OptimizerManager::OptimizerManager(
    const StringSet &enabledOpts, const StringSet &disabledOpts,
    ...
    : enabledOpts(trimOptimizerSuffix(enabledOpts)),
      disabledOpts(trimOptimizerSuffix(disabledOpts)), hllWriter(hllWriter),
      va(va), cio(cio), arithmExprEvaluator(arithmExprEvaluator),
      enableDebug(enableDebug), recoverFromOutOfMemory(true), backendRunOpts() {
 ...
 */
void OptimizerManager::runOptimizerProvidedItShouldBeRun(
    ShPtr<Optimizer> optimizer) {
  ...
  if (recoverFromOutOfMemory) {
    ...
    try {
      ...
      Log::error() << Log::Warning << "out of memory; trying to recover"
                   << std::endl;
...
void OptimizerManager::run(ShPtr<Module> m, Args &&...args) {
  auto optimizer =
      std::make_shared<Optimization>(m, std::forward<Args>(args)...);
