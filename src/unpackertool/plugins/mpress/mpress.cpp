...
const MpressUnpackerStubData
    mpressUnpackerStubData[MPRESS_UNPACKER_STUB_UNKNOWN] = {
        {0x2B6, 0x2BC, 0x2B8, 0x2C8, 0x2C0,
         COMPRESSION_LZMAT}, // MPRESS_UNPACKER_STUB_101_105
        {0x29E, 0x2A4, 0x2A0, 0x2B0, 0x2A8,
         COMPRESSION_LZMAT}, // MPRESS_UNPACKER_STUB_107_127
        {0x299, 0x29F, 0x29B, 0x2AB, 0x2A3,
         COMPRESSION_LZMAT}, // MPRESS_UNPACKER_STUB_201
        {0xB57, 0xB5D, 0xB59, 0xB61, 0xB69,
         COMPRESSION_LZMA}, // MPRESS_UNPACKER_STUB_205_LZMA
        {0x29C, 0x2A2, 0x29E, 0x2A6, 0x2AE,
         COMPRESSION_LZMAT}, // MPRESS_UNPACKER_STUB_205_LZMAT
        {0xB5A, 0xB60, 0xB5C, 0xB64, 0xB6C,
         COMPRESSION_LZMA}, // MPRESS_UNPACKER_STUB_212_219_LZMA
        {0x29F, 0x2A5, 0x2A1, 0x2A9, 0x2B1,
         COMPRESSION_LZMAT} // MPRESS_UNPACKER_STUB_212_219_LZMAT
};
...
MpressPlugin::MpressPlugin()
    : _file(), _peFile(nullptr), _unpackerStub(MPRESS_UNPACKER_STUB_UNKNOWN),
      _fixStub(MPRESS_FIX_STUB_UNKNOWN), _packedContentSect(nullptr),
      _addedSectionCount(0), _iatVa(0), _iatSize(0), _oepVa(0),
      _importHintsOffset(0) {
  info.name = "MPRESS";
 ...
 * Destructor.
 */
MpressPlugin::~MpressPlugin() { cleanup(); }
...
void MpressPlugin::prepare() {
  ...
  // We currently don't support PE32+ as the decompiler doesn't support them
  // anyways
...
void MpressPlugin::unpack() {
  ...
  _file->getEpSegment()->getBytes(
      packedContentSectAddrBytes,
      ep - _file->getEpSegment()->getAddress() +
          mpressUnpackerStubData[_unpackerStub].packedContentOffset,
      4);

  DynamicBuffer packedContentSectAddrBuffer(
      packedContentSectAddrBytes, _file->getFileFormat()->getEndianness());
  std::uint32_t packedContentSectAddr =
      ep + mpressUnpackerStubData[_unpackerStub].packedContentOffset +
      packedContentSectAddrBuffer.read<std::int32_t>(0);
  ...
  DynamicBuffer packedContentBuffer(packedContent,
                                    _file->getFileFormat()->getEndianness());
  ...
  std::uint32_t unpackedSize = packedContentBuffer.read<std::uint16_t>(0)
                               << 0xC;
  ...
  packedContentBuffer.erase(packedSize,
                            packedContentBuffer.getRealDataSize() - packedSize);

  // Decode the content of .MPRESS1 section which is compressed with LZMAT
  // algorithm
  DynamicBuffer unpackedContent(unpackedSize,
                                _file->getFileFormat()->getEndianness());
  ...
  // Split the unpacked section as much as possible into individual sections
  // based on the known offsets
...
bool MpressPlugin::decompressData(DynamicBuffer &compressedContent,
                                  DynamicBuffer &decompressedContent) {
  ...
  } else if (mpressUnpackerStubData[_unpackerStub].compression ==
             COMPRESSION_LZMA) {
...
void MpressPlugin::decodeLzmaProperties(DynamicBuffer &compressedContent,
                                        std::uint8_t &pb, std::uint8_t &lp,
                                        std::uint8_t &lc) {
...
std::uint32_t MpressPlugin::getFixStub() {
  ...
  // Fix imports stub is calculated from the EP section where there is offset
  // into it written at specific offset
  _file->getFileFormat()->getEpAddress(ep);
  epOffset = ep - _file->getEpSegment()->getAddress();
  _file->getEpSegment()->getBytes(
      fixStubOffsetBytes,
      epOffset + mpressUnpackerStubData[_unpackerStub].fixStubOffset, 4);

  DynamicBuffer fixStubOffsetBuffer(fixStubOffsetBytes,
                                    _file->getFileFormat()->getEndianness());

  // If we subtract the address of .MPRESS1 section, we have the offset in
  // section
  std::uint32_t fixImportsStubAddr =
      ep + mpressUnpackerStubData[_unpackerStub].fixStubOffset + 4 +
      fixStubOffsetBuffer.read<std::int32_t>(0);
...
void MpressPlugin::fixJumpsAndCalls(DynamicBuffer &buffer) {
  std::uint32_t pos = 0;
  std::uint32_t maxAddr =
      std::max(0, static_cast<std::int32_t>(buffer.getRealDataSize()) - 0x1000);
  while (pos < maxAddr) {
...
void MpressPlugin::fixImportsAndEp(DynamicBuffer &buffer) {
  ...
  // Fix import stub is then located at the <EP Address> + offset + <loaded
  // offset> This stub was packed together with code and data in .MPRESS1
  // section and also contains hints for import table rebuild
  std::uint32_t fixStubAddr = getFixStub();

  // Offet to the hints is located at the <Fix Import Stub Address> + offset
  // Import hints are located at the <Fix Import Stub Address> + offset +
  // <Import Hints Offset>
  std::uint32_t importHints =
      fixStubAddr + mpressFixStubData[_fixStub].importHintsOffset +
      buffer.read<std::uint32_t>(fixStubAddr +
                                 mpressFixStubData[_fixStub].importHintsOffset);

  // Go through MPRESS import hints and fill the import directory
  std::int32_t destOffset =
  ...
  while ((destOffsetDiff = buffer.read<std::int32_t>(readPos)) != -1) {
    ...
    buffer.writeRepeatingByte(0, readPos,
                              static_cast<uint32_t>(moduleName.length()));
    ...
    while (buffer.read<std::uint8_t>(readPos) != 0) {
      ...
      // Import by name
        ...
        buffer.writeRepeatingByte(
            0, readPos, static_cast<uint32_t>(symbolName.length() + 1));
  ...
  buffer.writeRepeatingByte(0, readPos,
                            4); // clear out last -1 from the hint list

  // Fix the import directory and import address directory addresses
  // Since ILT is lost we need to make them from scratch in the whole new
  // section Ensure the .imports section will be large enough, resize if there
  // are more data
  ...
  PeLib::PELIB_IMAGE_SECTION_HEADER *pNewSection =
      imageLoader.addSection(".imports", importSectSize);

  pNewSection->Characteristics = PeLib::PELIB_IMAGE_SCN_MEM_READ |
                                 PeLib::PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA;
  imageLoader.setDataDirectory(PeLib::PELIB_IMAGE_DIRECTORY_ENTRY_IMPORT,
                               pNewSection->VirtualAddress, importSectSize);

  // IAT needs to be put at the desired offset
  std::uint32_t iatOffset = lowestDestOffset;
  imageLoader.setDataDirectory(PeLib::PELIB_IMAGE_DIRECTORY_ENTRY_IAT,
                               iatOffset,
                               highestDestOffset + 4 - lowestDestOffset);
  ...
  std::uint32_t oepOffset =
      ...
      fixStubAddr + mpressFixStubData[_fixStub].importHintsOffset +
      buffer.read<std::uint32_t>(fixStubAddr +
                                 mpressFixStubData[_fixStub].oepOffset);
  imageLoader.setAddressOfEntryPoint(oepOffset);

  // Finally, get rid of the fix imports stub as it is going to mess up frontend
  // analysis in the unpacked section At the end we add 16 because of 4 bytes
  // directly belonging to the importHintsOffset and additional 12 bytes used to
  // store library calls GetProcAddress and LoadLibrary
  buffer.writeRepeatingByte(0, fixStubAddr,
                            mpressFixStubData[_fixStub].importHintsOffset + 16);
...
void MpressPlugin::offsetAnalysis(const DynamicBuffer &buffer) {
  ...
  PeLib::ImageLoader &imageLoader = _peFile->imageLoader();
  std::size_t packedContentSectionIndex =
      _packedContentSect->getSecSeg()->getIndex();
  std::uint32_t sectionAlignment = imageLoader.getSectionAlignment();
  std::uint32_t fixStubOffset = getFixStub();
  std::uint32_t dataFlags = PeLib::PELIB_IMAGE_SCN_MEM_WRITE |
                            PeLib::PELIB_IMAGE_SCN_MEM_READ |
                            PeLib::PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA;
  std::uint32_t codeFlags = dataFlags | PeLib::PELIB_IMAGE_SCN_CNT_CODE |
                            PeLib::PELIB_IMAGE_SCN_MEM_EXECUTE;

  // Get pointer to entry point section and also section with packed content
  packedContectSection =
      imageLoader.getSectionHeader(packedContentSectionIndex);
  entryPointSection = imageLoader.getSectionHeader(
      _file->getEpSegment()->getSecSeg()->getIndex());

  // Remove the .MPRESS2 section as the getEpSection()->getIndex() will be
  // shifted by newly created sections and we can do it safely here
  //_peFile->peHeader().removeSection(_file->getEpSection()->getIndex());
  // @todo There is some problem with removing a section and valid PE file in
  // some cases
  ...
  for (size_t i = _packedContentSect->getSecSeg()->getIndex() + 1;
       i < imageLoader.getNumberOfSections(); ++i) {
    PeLib::PELIB_IMAGE_SECTION_HEADER *tempSection =
        imageLoader.getSectionHeader(i);

    tempSection->PointerToRawData = tempSection->PointerToRawData + diff;
  }

  // Here we will split the big unpacked section into the more data sections and
  // try to locate the code section as much as possible We will use import
  // hints, IAT, fix stub address and OEP to locate the original sections We
  // know that hints and fix stub are located at the end of original sections We
  // also know that IAT is its own section
  std::uint32_t oepOffset = _oepVa - packedContectSection->VirtualAddress;
  std::uint32_t iatOffset =
      (_iatVa & ~(sectionAlignment - 1)) - packedContectSection->VirtualAddress;

  // We will need this as all other heuristics are based on the shrinking of
  // .text section
  std::uint32_t moveOffset = 0;

  // Hints are located before the OEP, we suspect that there is data section
  // before the code
  if (_importHintsOffset < oepOffset) {
    // Split the section into .data0|.text
    if (imageLoader.splitSection(
            packedContentSectionIndex, ".data0", ".text",
            (_importHintsOffset & ~(sectionAlignment - 1)) +
                sectionAlignment) == 0) {
      imageLoader.getSectionHeader(packedContentSectionIndex)->Characteristics =
          dataFlags;
      imageLoader.getSectionHeader(packedContentSectionIndex + 1)
          ->Characteristics = codeFlags;
      _addedSectionCount++;

      // We will need this as all other heuristics are based on the shrinking of
      // .text section We need to subtract this from every split offset
      // calculation
      moveOffset = imageLoader.getSectionHeader(packedContentSectionIndex)
                       ->SizeOfRawData;
    }

    // We don't mind if IAT is at the beginning of the data, since it is treated
    // properly as data However there can be IAT between data and code
    if (_importHintsOffset < iatOffset && iatOffset < oepOffset) {
      // Split the layout into .data0|.data2|.text
      if (imageLoader.splitSection(
              packedContentSectionIndex + 1, ".data2", ".text",
              ((iatOffset + _iatSize) & ~(sectionAlignment - 1)) +
                  sectionAlignment - moveOffset) == 0) {
        imageLoader.getSectionHeader(packedContentSectionIndex + 1)
            ->Characteristics = dataFlags;
        imageLoader.getSectionHeader(packedContentSectionIndex + 2)
            ->Characteristics = codeFlags;
        _addedSectionCount++;
    ...
    // Another heuristic is based on the fact that fix stub can be located
    // behind the code There we can get original code section almost perfectly
    if (fixStubOffset > oepOffset) {
      // Split into .data0|.text|.data1 or .data0|.data2|.text|.data1
      if (imageLoader.splitSection(
              packedContentSectionIndex + _addedSectionCount, ".text", ".data1",
              (fixStubOffset & ~(sectionAlignment - 1)) + sectionAlignment -
                  moveOffset) == 0) {
        imageLoader
            .getSectionHeader(packedContentSectionIndex + _addedSectionCount)
            ->Characteristics = codeFlags;
        imageLoader
            .getSectionHeader(packedContentSectionIndex + _addedSectionCount +
                              1)
  ...
  // The OEP is before the hints, so code section is probably the first one in
  // this big section
  else {
    // Split into .text|.data0
    if (imageLoader.splitSection(
            packedContentSectionIndex, ".text", ".data0",
            (_importHintsOffset & ~(sectionAlignment - 1)) +
                sectionAlignment) == 0) {
      imageLoader.getSectionHeader(packedContentSectionIndex)->Characteristics =
          codeFlags;
    ...
    // There can be even IAT between the .text and .data0 if the hints were
    // placed far behind
    ...
    // This will probably never happen because if there would be space for fix
    // stub, the hints would be there more probably, but just in case
    if (fixStubOffset < oepOffset) {
      // Split into .data1|.text|.data0 or .data1|.text|.data2|.data0
      if (imageLoader.splitSection(packedContentSectionIndex, ".data1", ".text",
                                   (fixStubOffset & ~(sectionAlignment - 1)) +
                                       sectionAlignment) == 0) {
        imageLoader.getSectionHeader(packedContentSectionIndex)
            ->Characteristics = dataFlags;
        imageLoader.getSectionHeader(packedContentSectionIndex + 1)
...
void MpressPlugin::trailingBytesAnalysis(const DynamicBuffer &buffer) {
  ...
  // 64 bytes at the every section alignment multiple are checked, if they are
  // all 0, the new section is probably here so it is created Only code section
  // left after this function is the one containing the OEP, the code will
  // execute even if some of the code is in data sections and decompiler will
  // take care of this in front-end instruction decoder
  PeLib::ImageLoader &imageLoader = _peFile->imageLoader();
  PeLib::PELIB_IMAGE_SECTION_HEADER *packedContentSection =
      imageLoader.getSectionHeader(_packedContentSect->getSecSeg()->getIndex());
  std::size_t section =
      imageLoader.getSectionIndexByRva(imageLoader.getAddressOfEntryPoint());
  PeLib::PELIB_IMAGE_SECTION_HEADER *entryPointSection =
      imageLoader.getSectionHeader(section);
  std::uint32_t sectionAlignment = imageLoader.getSectionAlignment();
  std::uint32_t startOffset = entryPointSection->PointerToRawData -
                              packedContentSection->PointerToRawData;
  std::uint32_t endOffset = startOffset + entryPointSection->SizeOfRawData;
  std::uint32_t oepOffset = imageLoader.getAddressOfEntryPoint() -
                            packedContentSection->VirtualAddress;

  // Used to build the section names
  std::uint32_t nameCounter = 3;

  // Start at the next section alignment closest to the start of current .text
  // section Move with the step of section alignment End if end of the unpacked
  // section is reached
  for (std::uint32_t offset = startOffset + sectionAlignment;
       offset < endOffset; offset += sectionAlignment) {
    // Inspect trailing 64 bytes, if they are all 0, needSplit is marked as
    // true, resulting in section split
    bool needSplit = true;
    ...
    if (startOffset <= oepOffset && oepOffset < offset) {
      ...
      flags[1] = PeLib::PELIB_IMAGE_SCN_MEM_WRITE |
                 PeLib::PELIB_IMAGE_SCN_MEM_READ |
                 PeLib::PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA;
      flags[0] = flags[1] | PeLib::PELIB_IMAGE_SCN_MEM_EXECUTE |
    ...
    // OEP lies in the second part of splitted sections
      ...
      ssSecond << ".text";
      flags[0] = PeLib::PELIB_IMAGE_SCN_MEM_WRITE |
                 PeLib::PELIB_IMAGE_SCN_MEM_READ |
                 PeLib::PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA;
      flags[1] = flags[0] | PeLib::PELIB_IMAGE_SCN_MEM_EXECUTE |
                 PeLib::PELIB_IMAGE_SCN_CNT_CODE;
    }
    // OEP doesn't lie in neither of these two parts, this can happen if .text
    // section was already created and we are analyzing the rest after that
    else {
      ssFirst << ".data" << nameCounter++;
      ssSecond << ".data" << nameCounter++;
      flags[0] = PeLib::PELIB_IMAGE_SCN_MEM_WRITE |
    ...
    imageLoader.splitSection(section, ssFirst.str(), ssSecond.str(),
                             offset - startOffset);
...
void MpressPlugin::fixRelocations() {
  ...
  epSegment->getBytes(
      relocRvaBytes,
      epAddress + mpressUnpackerStubData[_unpackerStub].relocOffset, 4);
  epSegment->getBytes(
      relocSizeBytes,
      epAddress + mpressUnpackerStubData[_unpackerStub].relocSizeOffset, 4);

  DynamicBuffer relocRvaBuffer(relocRvaBytes,
                               _file->getFileFormat()->getEndianness());
  DynamicBuffer relocSizeBuffer(relocSizeBytes,
                                _file->getFileFormat()->getEndianness());
  ...
  imageLoader.setDataDirectory(PeLib::PELIB_IMAGE_DIRECTORY_ENTRY_BASERELOC,
                               relocRva, relocSize);
}
...
MpressUnpackerStub MpressPlugin::detectUnpackerStubVersion() {
  ...
  DynamicBuffer signatureBuffer(signatureBytes,
                                _file->getFileFormat()->getEndianness());
  ...
  for (std::uint32_t version = 0;
       version < static_cast<std::uint32_t>(MPRESS_UNPACKER_STUB_UNKNOWN);
       ++version) {
...
MpressFixStub
MpressPlugin::detectFixStubVersion(DynamicBuffer &unpackedContent) {
  std::uint32_t fixStub = getFixStub();
  for (std::uint32_t version = 0;
       version < static_cast<std::uint32_t>(MPRESS_FIX_STUB_UNKNOWN);
       ++version) {
    if (unpackedContent.read<std::uint8_t>(fixStub + 7) ==
        mpressFixStubData[version].signature) {
...
void MpressPlugin::saveFile(const std::string &fileName,
                            DynamicBuffer &content) {
  PeLib::ImageLoader &imageLoader = _peFile->imageLoader();
  ...
  // Copy the section bytes from original file for the sections preceding the
  // packed section
  for (std::uint32_t index = 0;
       index < _packedContentSect->getSecSeg()->getIndex(); ++index)
    copySectionFromOriginalFile(index, fileName, index);

  // Copy the section bytes in between packed section and EP section
  for (std::uint32_t index =
           _packedContentSect->getSecSeg()->getIndex() + _addedSectionCount;
       index < _file->getEpSegment()->getSecSeg()->getIndex(); ++index)
    copySectionFromOriginalFile(index, fileName, index + _addedSectionCount);

  // Copy the section bytes from original file for sections after EP section
  // excluded
  for (std::uint32_t index = _file->getEpSegment()->getSecSeg()->getIndex() + 1;
       index < _file->getNumberOfSegments(); ++index)
    copySectionFromOriginalFile(index, fileName, index + _addedSectionCount);

  // Write content of new import section
  std::uint32_t Rva =
      imageLoader.getDataDirRva(PeLib::PELIB_IMAGE_DIRECTORY_ENTRY_IMPORT);
  _peFile->impDir().write(fileName, imageLoader.getFileOffsetFromRva(Rva), Rva,
                          imageLoader.getPointerSize());
  ...
  for (size_t fileIndex = 0;
       fileIndex < _peFile->impDir().getNumberOfFiles(PeLib::OLDDIR);
       ++fileIndex) {
    auto tmp = static_cast<unsigned int>(fileIndex);
    auto w = static_cast<unsigned short>(
        _packedContentSect->getSecSeg()->getIndex());
    size_t destOffset = _peFile->impDir().getFirstThunk(tmp, PeLib::OLDDIR) -
                        imageLoader.getSectionHeader(w)->VirtualAddress;
    for (size_t funcIndex = 0;
         funcIndex < _peFile->impDir().getNumberOfFunctions(tmp, PeLib::OLDDIR);
         ++funcIndex, destOffset += 4) {
      content.write<std::uint32_t>(
          _peFile->impDir().getOriginalFirstThunk(
              tmp, static_cast<unsigned int>(funcIndex), PeLib::OLDDIR),
  ...
  outputFile.seekp(
      imageLoader.getSectionHeader(_packedContentSect->getSecSeg()->getIndex())
          ->PointerToRawData,
      std::ios_base::beg);
  outputFile.write(reinterpret_cast<const char *>(content.getRawBuffer()),
                   content.getRealDataSize());
  outputFile.close();
}

void MpressPlugin::copySectionFromOriginalFile(std::uint32_t origSectIndex,
